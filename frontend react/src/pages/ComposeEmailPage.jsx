...
export default function ComposeEmailPage({ onNavigate }) {
  ...
  const selectedTemplate = useMemo(
    () => mockTemplates.find((t) => String(t.id) === String(templateId)),
    [templateId]
  );

  // Versões disponíveis para o template selecionado
  const versions = selectedTemplate ? selectedTemplate.versions : [];

  // Versão selecionada (ou mais recente)
  const selectedVersion = useMemo(() => {
    if (!selectedTemplate) return null;
    if (templateVersion) {
      return selectedTemplate.versions.find((v) => v.version === templateVersion) || selectedTemplate.versions.at(-1);
    }
    return selectedTemplate.versions.at(-1);
  }, [selectedTemplate, templateVersion]);

  // Variáveis da versão selecionada
  ...
  const handleTemplateChange = (e) => {
    ...
    setTemplateVersion("");
    setVariableValues({});
    const tmpl = mockTemplates.find((t) => String(t.id) === String(id));
    if (tmpl) {
      const lastVer = tmpl.versions.at(-1);
      setSubject(lastVer?.subject || "");
    } else {
      setSubject("");
    }
  };
  ...
  const handleVersionChange = (e) => {
    ...
    const ver = selectedTemplate?.versions.find((x) => x.version === v);
    if (ver) setSubject(ver.subject);
  };
  ...
  const handleSend = (e) => {
    e.preventDefault();
    // Simulação: apenas exibe confirmação
    setSent(true);
  };
      ...
      <form onSubmit={handleSend} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <section style={{ background: COLORS.surface, borderRadius: RADIUS.lg, border: `1px solid ${COLORS.border}`, padding: "24px 28px", boxShadow: SHADOW.card, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <FieldGroup label="Versão" hint={selectedVersion ? `Usando v${selectedVersion.version}` : ""}>
              <select
              ...
              >
                ...
                {versions.map((v) => (
                  <option key={v.version} value={v.version}>v{v.version} — {v.updatedAt}</option>
                ))}
              </select>
            </FieldGroup>
          </div>
          ...
          {selectedVersion && (
            <div style={{
              ...
              <div style={{ fontSize: 13, color: COLORS.textSecondary, marginBottom: 4 }}>
                <strong>Assunto:</strong> {selectedVersion.subject}
              </div>
              <div
                style={{ fontSize: 13, color: COLORS.textSecondary, lineHeight: 1.6, borderTop: `1px solid ${COLORS.borderLight}`, paddingTop: 10, marginTop: 8 }}
        ...
        <section style={{ background: COLORS.surface, borderRadius: RADIUS.lg, border: `1px solid ${COLORS.border}`, padding: "24px 28px", boxShadow: SHADOW.card, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: 12, color: COLORS.textMuted }}>Limite: 10MB total</span>
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 12 }}>
          ...
          >
            ...
            {sendMode === "immediate" ? "Enviar Agora" : "Agendar Envio"}
          </button>
        </div>
      </form>
    </div>
