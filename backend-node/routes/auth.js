...
//   4. DELETE /api/auth/usuarios/:id → exclui um usuário
// ============================================
...
// ── EXCLUIR USUÁRIO ──────────────────────────
// O frontend manda o id na URL: /usuarios/5
// Se o usuário não existir, devolve erro 404.
router.delete("/usuarios/:id", async (req, res) => {
  ...
  try {
    ...
    // Nenhuma linha apagada? O id não existe.
    if (resultado.rows.length === 0) {
    ...
    // Código 23503 = o usuário tem emails ligados a ele
    // (regra FOREIGN KEY do banco impede apagar)
    if (erro.code === "23503") {
      return res.status(409).json({
        erro: "Este usuário possui emails no histórico e não pode ser excluído.",
      });
    }
    console.error(erro);
    res.status(500).json({ erro: "Erro no servidor." });
